...
const LANGUAGE_SKILLS = [
  { label: "Pronunciation", value: 72, color: "#F97316", textColor: "text-orange-500" },
  { label: "Meaning", value: 87, color: "#22C55E", textColor: "text-green-500" },
  { label: "Etymology (Roots)", value: 58, color: "#06B6D4", textColor: "text-cyan-500" },
  { label: "Spelling", value: 65, color: "#F97316", textColor: "text-orange-500" },
];
...
export default function ProgressPage() {
  ...
  const maxWeeklyXp = Math.max(...WEEKLY_XP.map((d) => d.xp));
  ...
  return (
    <div className="space-y-4 pb-10 animate-fade-in">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {children.map((c, i) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-base font-semibold transition-all duration-200 ${i === selectedIdx
                ? "bg-white shadow-md border border-gray-100 text-[#14062B]"
                : "text-gray-500 hover:bg-white/70"
                }`}
            >
              <span
                className="w-2.5 h-2.5 rounded-full inline-block"
                style={{ backgroundColor: TAB_DOT_COLORS[i % TAB_DOT_COLORS.length] }}
              />
              {c.name}
            </button>
          ))}
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
            <StatItem icon={<Trophy size={18} className="text-blue-500" />} value={`Lv. ${child.current_level}`} label="Level" />
            <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<Zap size={18} className="text-yellow-500" />} value={child.xp.toLocaleString()} label="Total XP" />
            <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<CheckCircle2 size={18} className="text-green-500" />} value="42" label="Quests" />
            {/* <div className="w-px h-8 bg-gray-100 hidden sm:block" /> */}
            {/* <StatItem icon={<BookOpen size={18} className="text-indigo-500" />} value="128" label="Words Mastered" /> */}
            {/* <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<AlertCircle size={18} className="text-red-500" />} value="14" label="Needs Review" /> */}
            <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<Target size={18} className="text-orange-500" />} value="87%" label="Accuracy" />
            {/* <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<Calendar size={18} className="text-cyan-500" />} value="4.2 h" label="Weekly Practice" /> */}
            <div className="w-px h-8 bg-gray-100 hidden sm:block" />
            <StatItem icon={<Flame size={18} className="text-orange-600" />} value={String(child.streak_days)} label="Day Streak" />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Language Learning (static for now) */}
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500">
              <Languages size={24} />
            </div>
            <h3 className="text-base font-bold text-[#14062B]">Language Learning</h3>
          </div>
            ...
            {LANGUAGE_SKILLS.map((skill) => (
              <div key={skill.label}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-xs font-bold ${skill.textColor}`}>{skill.value}%</span>
                </div>
        ...
        {/* Weekly XP Activity (static for now) */}
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
          ...
          <div className="flex items-end justify-between gap-1 h-28 pb-1">
            {WEEKLY_XP.map((d, i) => {
              const isToday = i === WEEKLY_XP.length - 1;
              const heightPct = (d.xp / maxWeeklyXp) * 100;
              return (
