...
import { Plus, Flame, Zap, Target, Star, MoreVertical } from "lucide-react";

const children = [
  {
    id: "1",
    name: "Rahul Kumar",
    username: "@rahul_wizard",
    status: "Active",
    grade: "5th",
    age: 10,
    joined: "Jan 12, 2025",
    ...
    <div className="space-y-6 pb-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-dashboard-text-muted mt-1 font-medium">2 linked profiles</p>
        </div>
        <Link 
      ...
      <div className="space-y-5">
        {children.map((child) => (
          <div key={child.id} className="bg-white rounded-[2rem] border border-dashboard-border p-6 shadow-sm hover:shadow-lg transition-all duration-300 group relative">
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div className="flex items-start gap-5">
                <div className={`w-14 h-14 rounded-full ${child.avatarColor} flex items-center justify-center text-white border-4 border-white shadow-lg group-hover:scale-110 transition-transform flex-shrink-0`}>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-dashboard-text-muted font-medium">
                    {child.grade} Grade • Age {child.age} • Joined {child.joined}
                  </p>
                  
                  <div className="flex items-center gap-6 pt-2">
                    <div className="flex items-center gap-1.5">
                      <Flame size={14} className="text-orange-500" />
                      <span className="text-xs font-bold text-[#14062B]">{child.streak} days streak</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-bold text-[#14062B] whitespace-nowrap">{child.accuracy}% accuracy</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Star size={14} className="text-dashboard-purple" />
                      <span className="text-xs font-bold text-[#14062B]">Level {child.level}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <div className="bg-[#FCFAFF] px-5 py-3 rounded-2xl border-2 border-dashed border-purple-100 flex flex-col items-center min-w-[120px]">
                  ...
                  <span className="text-base font-black text-dashboard-purple tracking-widest font-syne">{child.inviteCode}</span>
